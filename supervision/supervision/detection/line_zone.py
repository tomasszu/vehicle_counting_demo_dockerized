...
class LineZone:
    ...
    def __init__(
        ...
        #MODIF -----------------------------------------------------------------
        self.possible_cross_state: Dict[str, bool] = {}
        #Atskaites counter, cik frames pagajis kops masina nomainija puses un pazuda
        self.possible_cross_counter: Dict[str, int] = {}
    ...
    #MODIF ------------------------------------------------------------------------------------------------------
    def possible_cross_state_check(self, detections):
        #Parbaudam possible cross state - vai objekts daleji nepargaja linijai un pazuda pirms pilnigas parejas
        # Ejam cauri pieregistretajiem iespejamajiem linijas parbraucieniem
        for item in self.possible_cross_state.items():
            # ID un puse
            tracker_id , tracker_state = item
            # ja apskatamais vehicle nav detekteets kadraa
            if detections.empty() or tracker_id not in detections.tracker_id:
                # ja apskatamais vehicle ir novietojies otraa linijas pusee kops sakuma
                if self.tracker_state[tracker_id] is not self.possible_cross_state[tracker_id]:
                    # ja sis pabrauciens jau ir bijis registrets
                    if tracker_id in self.possible_cross_counter:
                        # ja pagajis jau counter frame skaits kops vehicle pazuda
                        if self.possible_cross_counter[tracker_id] > self.counter_value:
                            print("Out of frame vehicle established as passed through")
                            # saglabaa pusi kuraa palika
                            tracker_state = self.possible_cross_state[tracker_id]
                            if tracker_state:
                                self.in_count += 1
                            else:
                                self.out_count += 1
                            # izdzes ierakstu
                            self.possible_cross_state.pop(tracker_id)
                            self.possible_cross_counter.pop(tracker_id)
                            #izdzesaam, varam iet taalaak
                            break
                        else:
                            #pieskaitam counterim
                            self.possible_cross_counter[tracker_id] += 1
                    else:
                        #resetojam counteri
                        self.possible_cross_counter[tracker_id] = 0
                else:
                    # ja pareja jau ir pieregistreta pilnibaa, tracked stateaaa
                    self.possible_cross_state.pop(tracker_id)
                    if tracker_id in self.possible_cross_counter:
                        self.possible_cross_counter.pop(tracker_id)
                    #izdzesaam, varam iet taalaak
                ...
                #ja vehicle atkal uzrodaas, atiestatam taimeri
                if tracker_id in self.possible_cross_counter:
                    self.possible_cross_counter[tracker_id] = 0
    #------------------------------------------------------------------------------------------------------------------
    ...
    def trigger(self, detections: Detections) -> Tuple[np.ndarray, np.ndarray]:
        ...
        # #Test print
        # print("possible cross state:------------------------------------")
        # print(self.possible_cross_state)
        # print("cross count:------------------------------------")
        # print(self.possible_cross_counter)
        ...
        if len(detections) == 0:
            #MODIFIKACIJA -----------------------------------------------------------------
            self.possible_cross_state_check(detections)
            #-----------------------------------------------------------------------------
        ...
        for i, tracker_id in enumerate(detections.tracker_id):
            ...
            # print("--------------------------------------------------")
            # for anchor in box_anchors:
            #     print(self.vector.distance_point_to_line(point=anchor))

            # print(self.vector.furthest_anchor_id_from_vector(box_anchors=box_anchors))
            ...
            # Tatad tie trackers ir tas 4 true or false vertibas

            #Tracker state registres so masinu kad vina vel nav uzbraukusi linijai virsuu un iestatis uz true vai false (viena vai otra pusee)
            ...
            if len(set(triggers)) == 2:
                #MODIFIKACIJA ---------------------------------------
                # Pieskiram provizorisko tracker state, skatoties kura bbox puse ir vairaak aiz liinijas
                trigger_id = self.vector.furthest_anchor_id_from_vector(box_anchors=box_anchors)
                tracker_state = triggers[trigger_id]
                #Ja nav jau uzregistrets tad ieregistre kuraa pusee sitais tracking ID objekts atrodas
                if tracker_id not in self.tracker_state:
                    self.tracker_state[tracker_id] = tracker_state
                #Ja ir jau uzregestrets un palielam ir pargajis otraa pusee, bet veel ir uz liinijas
                else:
                    if self.tracker_state[tracker_id] is not tracker_state and tracker_id not in self.possible_cross_state:
                        self.possible_cross_state[tracker_id] = tracker_state
                        #print("possible cross", tracker_id)
                #----------------------------------------------------
                continue
            
            # Ja nav divas vienaa linijas pusee un divas otraa tad tiek šiten talu
            ...
            #Ja tads jau bija registrets un veljoprojam ir taja puse tad turpinam
            if self.tracker_state.get(tracker_id) == tracker_state:
                continue

            #Ja bija registrets bet nesakriit, pieskiram jauno pusi
            self.tracker_state[tracker_id] = tracker_state
            if tracker_state:
                self.in_count += 1
                crossed_in[i] = True
